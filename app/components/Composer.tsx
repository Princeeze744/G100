...
export function Composer({ uid, me, onPosted }: { uid: string; me: any; onPosted: () => void }) {
  ...
  async function submit() {
    ...
    for (const file of files) {
      ...
      const blob: Blob = isVid ? file : await shrinkImage(file, 1400);
      const path = uid + "/" + Date.now() + "-" + Math.random().toString(36).slice(2, 7) + (isVid ? ".mp4" : ".jpg");
      const { error: upErr } = await supabase.storage.from("posts").upload(path, blob, { contentType: isVid ? file.type : "image/jpeg" });
      ...
      if (isVid) video = url; else urls.push(url);
    }
    const { error } = await supabase.from("posts").insert({ author_id: uid, body: body.trim(), image_url: urls[0] || "", image_urls: urls, video_url: video });
        ...
        <div className="flex-1">
          ...
          <div className="mt-3 flex items-center justify-between">
            <button onClick={() => fileRef.current?.click()} disabled={files.length >= 4} className="text-xs text-neutral-400 transition hover:text-[var(--eye)] disabled:opacity-40">
              + Media {files.length > 0 ? "(" + files.length + "/4)" : ""}
            </button>
            <input ref={fileRef} type="file" accept="image/*,video/*" multiple className="hidden" onChange={(e) => { if (e.target.files?.length) addFiles(e.target.files); e.target.value = ""; }} />
